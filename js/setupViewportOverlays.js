...
function setupViewportOverlays(element, data) {
    ...
    $(topLeft[2]).text(ConvertDate(data.patientBD));
    $(topRight[0]).text(data.studyDescription);
    $(topRight[1]).text(ConvertDate(data.studyDate));
    // Tendria que ver que serie esta seleccionanda en este viewPort.
    $(bottomMiddle[0]).text(data.seriesList[0].institutionName);
